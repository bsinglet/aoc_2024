...
def load_input_2(filename: str) -> list:
    my_lines = ''
    machines = list()
    with open(filename, 'r') as my_file:
        my_lines = my_file.read()
    ...
    for each_machine in my_lines:
        ...
        temp_machine['B_x'] = int(re.match('[^\\:]+:\\s+X\\+(\\d+),\\s+Y\\+(\\d+)', each_machine[1]).group(1))
        temp_machine['B_y'] = int(re.match('[^\\:]+:\\s+X\\+(\\d+),\\s+Y\\+(\\d+)', each_machine[1]).group(2))
        temp_machine['P_x'] = int(re.match('[^\\:]+:\\s+X=(\\d+),\\s+Y=(\\d+)', each_machine[2]).group(1)) + 10000000000000
        temp_machine['P_y'] = int(re.match('[^\\:]+:\\s+X=(\\d+),\\s+Y=(\\d+)', each_machine[2]).group(2)) + 10000000000000
        machines.append(temp_machine)
    return machines
...
def main():
    filename = '..\\inputs\\day13_short.txt'
    ...
    machines = load_input_2(filename=filename)
    result_1 = find_minimum_tokens(machines)
    print(f'The fewest tokens you would have to spend to win all possible prizes is {result_1}.')
...
class TestDay12Methods(unittest.TestCase):
    ...
    def test_find_minimum_tokens_part_2_short(self):
        self.assertEqual(find_minimum_tokens(load_input_2(filename='..\\inputs\\day13_short.txt')), 875318608908)
    
    def test_find_minimum_tokens_part_2_full(self):
        self.assertEqual(find_minimum_tokens(load_input_2(filename='..\\inputs\\day13.txt')), 105620095782547)
